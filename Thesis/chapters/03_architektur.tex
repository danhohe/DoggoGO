...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
      ...
      label/.style={font=\footnotesize\itshape, midway, above}
    ]
    \node[comp] (ios)     at (0, 1.2)  {iOS-App\\(Swift/SwiftUI)};
    \node[comp] (angular) at (0,-1.2)  {Angular\\Web-App};
    \node[comp] (api)     at (4.5, 0)  {Backend-API\\(ASP.NET Core 8)};
    \node[comp] (db)      at (9, 0)    {PostgreSQL\\Datenbank};

    \draw[arr] (ios)     -- node[label]{HTTP/JSON} (api);
    \draw[arr] (angular) -- node[label, below]{HTTP/JSON} (api);
    \draw[arr] (api)     -- node[label]{EF Core} (db);
  \end{tikzpicture}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[layer] (ctrl) at (0, 2.2) {Controller-Schicht\\(HTTP-Requests, Routing)};
    \node[layer] (data) at (0, 1.1) {Datenbankschicht\\(DoggoContext / EF Core 9)};
    \node[layer] (pg)   at (0, 0)   {PostgreSQL};
    \node[util]  (seed) at (5, 1.1) {BreedSeeder\\(Utility)};
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[lifeline] (db)  at (11,0)  {PostgreSQL};

    % Lebenslinien
    \draw[dotted] (ios.south) -- (0,-7.5);
    \draw[dotted] (api.south) -- (6,-7.5);
    \draw[dotted] (db.south)  -- (11,-7.5);

    % Nachrichten
    \draw[msg] (0,-0.8) -- node[lbl]{POST /api/owners/login} (6,-0.8);
    \draw[msg] (6,-1.4) -- node[lbl]{SELECT Owner WHERE email} (11,-1.4);
    \draw[ret] (11,-2.0) -- node[lbl]{Owner-Datensatz} (6,-2.0);
    \draw[ret] (6,-2.6) -- node[lbl]{200 OK + JWT} (0,-2.6);
    \node[font=\footnotesize\itshape] at (1.5,-3.1) {$\rightarrow$ Token in Keychain};
    \draw[msg] (0,-3.7) -- node[lbl]{GET /api/dogs (Bearer Token)} (6,-3.7);
    \draw[msg] (6,-4.3) -- node[lbl]{JWT validieren + SELECT Dogs} (11,-4.3);
    \draw[ret] (11,-4.9) -- node[lbl]{Dog-Datensätze} (6,-4.9);
    \draw[ret] (6,-5.5) -- node[lbl]{200 OK + Dogs\-Liste (JSON)} (0,-5.5);
    \node[font=\footnotesize\itshape] at (1.5,-6.0) {$\rightarrow$ UI aktualisieren};
  \end{tikzpicture}
