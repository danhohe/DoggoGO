...
\begin{lstlisting}[language=csharp, caption={Modell Owner.cs}, label=lst:owner, float=htbp]
public class Owner
{
...
\begin{lstlisting}[language=csharp, caption={Modell Dog.cs}, label=lst:dog, float=htbp]
public class Dog
{
    public Guid Id { get; set; }
    public string Name { get; set; }
...
\begin{lstlisting}[language=csharp, caption={Modelle Breed.cs und LegalRestrictions.cs}, label=lst:breed, float=htbp]
public class Breed
{
    public Guid Id { get; set; }
...
\begin{lstlisting}[language=csharp, caption={DoggoContext.cs – Beziehungskonfiguration (Auszug)}, label=lst:context, float=htbp]
...
\begin{lstlisting}[language=csharp, caption={BreedSeeder.cs – Idempotentes Seeding}, label=lst:seeder, float=htbp]
...
\begin{lstlisting}[language={}, caption={breeds.json – Auszug (Bullterrier)}, label=lst:breeds-json, float=htbp]
{
...
\begin{lstlisting}[language=csharp, caption={Passwort-Hashing (Registrierung, Auszug)}, label=lst:hash, float=htbp]
...
\begin{lstlisting}[language=csharp, caption={JWT-Ausstellung (Login, Auszug)}, label=lst:jwt, float=htbp]
var claims = new[]
{
...
\begin{lstlisting}[language=csharp, caption={Program.cs (vereinfacht)}, label=lst:program, float=htbp]
...
\begin{lstlisting}[language={}, caption={Dockerfile – Multi-Stage-Build (Auszug)}, label=lst:dockerfile, float=htbp]
...
\begin{lstlisting}[language={}, caption={compose.yaml}, label=lst:compose, float=htbp]
version: '3.8'
services:
  postgres:
